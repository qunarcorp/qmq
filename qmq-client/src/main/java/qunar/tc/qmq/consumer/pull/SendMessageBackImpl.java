...
class SendMessageBackImpl implements SendMessageBack {
    ...
    @Override
    public void sendBack(final BrokerGroupInfo brokerGroup, BaseMessage message, final Callback callback,
        ...
        final Datagram datagram = RemotingBuilder
                .buildRequestDatagram(SEND_MESSAGE, new MessagesPayloadHolder(Lists.newArrayList(message)));
            ...
            NETTY_CLIENT.sendAsync(brokerGroup.getMaster(), datagram, SEND_MESSAGE_TIMEOUT,
                    new ResponseFuture.Callback() {
                        ...
                        private void handleResponse(final Datagram response) {
                            ...
                            if (respCode == CommandCode.SUCCESS
                                    && sendResult.getCode() == MessageProducerCode.SUCCESS) {
                                callback.success();
                                return;
                            }

                            if (respCode == CommandCode.BROKER_REJECT
                                    || sendResult.getCode() == MessageProducerCode.BROKER_READ_ONLY) {
                                brokerGroup.setAvailable(false);
                                brokerService.refreshMetaInfo(clientType, subject);
                            }

                            monitorSendError(subject, 100 + respCode);
                            callback.fail(new SendMessageBackException(
                                    "responseCode=" + respCode + ", sendCode=" + sendResult.getCode()));
                        }
                    });
    ...
    @Override
    public void sendBackAndCompleteNack(final String brokerGroupName, final int nextRetryCount,
            final BaseMessage message, final AckEntry ackEntry) {
        String subject = message.getSubject();
        String partitionName = message.getPartitionName();
        String messageId = message.getMessageId();
        OrderStrategy orderStrategy = OrderStrategyCache.getStrategy(subject);
        final BrokerClusterInfo brokerCluster = brokerService.getProducerBrokerCluster(ClientType.PRODUCER,
                subject);
        final SendMessageBack.Callback callback = new SendMessageBack.Callback() {
            ...
            @Override
            public void success() {
                LOGGER.info("send message back successfully subject {} partitionName {} messageId {} retries {}", subject,
                        partitionName, messageId, nextRetryCount);
                ackEntry.completed();
            }

            @Override
            public void fail(Throwable e) {
                MessageGroup messageGroup = orderStrategy.resolveMessageGroup(message);
                if (retryNumOnFail.incrementAndGet() > retryTooMuch) {
                    if (e instanceof SendMessageBackException) {
                        LOGGER.error(
                                "send message back fail subject {} partitionName {} messageId {} retries {} , and retry {} times after {} seconds. exception: {}",
                                subject, partitionName, messageId, nextRetryCount, retryNumOnFail.get(), SEND_BACK_DELAY_SECONDS,
                                e.getMessage());
                    } else {
                        LOGGER.error(
                                "send message back fail subject {} partitionName {} messageId {} retries {}, and retry {} times after {} seconds",
                                subject, partitionName, messageId, nextRetryCount, retryNumOnFail.get(), SEND_BACK_DELAY_SECONDS,
                                e);
                    }
                    ...
                    TimerUtil.newTimeout(new TimerTask() {
                        ...
                        public void run(Timeout timeout) {
                            SendMessageBackImpl.this
                                    .sendBackAndCompleteNack(messageGroup.getBrokerGroup(), message, callback1);
                        }
                    }, SEND_BACK_DELAY_SECONDS, TimeUnit.SECONDS);
                } else {
                    if (e instanceof SendMessageBackException) {
                        LOGGER.error("send message back fail, and retry {} times after {} seconds. exception: {}",
                                retryNumOnFail.get(), SEND_BACK_DELAY_SECONDS, e.getMessage());
                    } else {
                        LOGGER.error("send message back fail, and retry {} times after {} seconds",
                                retryNumOnFail.get(), SEND_BACK_DELAY_SECONDS, e);
                    }
                    SendMessageBackImpl.this.sendBackAndCompleteNack(messageGroup.getBrokerGroup(), message, this);
                }
            }
        };
    ...
    private void sendBackAndCompleteNack(final String brokerGroupName, final BaseMessage message,
            final SendMessageBack.Callback callback) {
        final BrokerClusterInfo brokerCluster = brokerService
                .getProducerBrokerCluster(ClientType.PRODUCER, message.getSubject());
        final BrokerGroupInfo brokerGroup = brokerCluster.getGroupByName(brokerGroupName);
        sendBack(brokerGroup, message, callback, ClientType.PRODUCER);
    }

    private static void monitorSendError(String subject, int errorCode) {
        Metrics.counter("qmq_pull_send_msg_error", new String[]{"subject", "error"},
                new String[]{subject, String.valueOf(errorCode)}).inc();
    }
}

...
public class PullConsumerFactory {
    ...
    public PullConsumer getOrCreateDefault(String subject, String consumerGroup, boolean isBroadcast) {
        ...
        try {
            ...
            PullConsumer consumerImpl = createDefaultPullConsumer(realSubject, consumerGroup, isBroadcast);
            pullConsumerMap.put(key, consumerImpl);
            return consumerImpl;
        } catch (Exception e) {
            if (e instanceof DuplicateListenerException) {
                throw new CreatePullConsumerException("已经使用了onMessage方式处理的主题不能再纯拉模式", realSubject, consumerGroup);
            }
            throw e;
        } finally {

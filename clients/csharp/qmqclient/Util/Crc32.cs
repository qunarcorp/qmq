...
namespace Qunar.TC.Qmq.Client.Util
{
    ...
    internal sealed class Crc32
    {
        ...
        private void Update(byte[] bytes, int offset, int size)
        {
            ...
            while (size > 7)
            {
                uint c0 = (bytes[offset + 0] ^ localCrc) & 0xff;
                uint c1 = (bytes[offset + 1] ^ (localCrc >>= 8)) & 0xff;
                uint c2 = (bytes[offset + 2] ^ (localCrc >>= 8)) & 0xff;
                uint c3 = (bytes[offset + 3] ^ (localCrc >>= 8)) & 0xff;
                localCrc = (T[T8_7_IDEX, c0] ^ T[T8_6_IDEX, c1]) ^ (T[T8_5_IDEX, c2] ^ T[T8_4_IDEX, c3]);

                int c4 = bytes[offset + 4] & 0xff;
                int c5 = bytes[offset + 5] & 0xff;
                int c6 = bytes[offset + 6] & 0xff;
                int c7 = bytes[offset + 7] & 0xff;
                localCrc ^= (T[T8_3_IDEX, c4] ^ T[T8_2_IDEX, c5]) ^ (T[T8_1_IDEX, c6] ^ T[T8_0_IDEX, c7]);
            ...
            while (size > 0)
            {
                localCrc = (localCrc >> 8) ^ T[T8_0_IDEX, (localCrc ^ bytes[offset++]) & 0xff];
                size--;
            }
        ...
        {
            crc = (crc >> 8) ^ T[T8_0_IDEX, (crc ^ input) & 0xff];
        }
        ...
        /// <param name="offset">相对于stream start</param>
        /// <param name="size"></param>
        /// <returns></returns>
        private static byte[] StreamToBytes(Stream stream, int offset, int size)
        {
            if (size + offset > stream.Length)
            {
                throw new ArgumentOutOfRangeException("size + offset > stream.Length");
            }
        ...
        public static long GetCRC32(byte[] bytes, int offset, int size)
        {
            Crc32 crc = new Crc32();
        ...
        public static long GetCRC32(Stream stream, int offset, int size)
        {
            Crc32 crc = new Crc32();
            byte[] bytes = StreamToBytes(stream, offset, size);
            crc.Update(bytes, 0, size);
            return crc.GetValue();
        }

        #region  CRC-32 lookup tables 
        private static readonly int T8_0_IDEX = 0;
        private static readonly int T8_1_IDEX = 1;
        private static readonly int T8_2_IDEX = 2;
        private static readonly int T8_3_IDEX = 3;
        private static readonly int T8_4_IDEX = 4;
        private static readonly int T8_5_IDEX = 5;
        private static readonly int T8_6_IDEX = 6;
        private static readonly int T8_7_IDEX = 7;

        //Crc Table
        private static readonly UInt32[,] T = new UInt32[8, 256]
        {                         
            /* T8_0 */
            {

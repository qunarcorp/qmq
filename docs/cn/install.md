...
### 最佳配置
metaserver的要求不高，一般压力也不大，主要就是处理一些心跳请求，应用是无状态的，除了写日志无任何本地磁盘操作，使用虚拟机和docker皆可。
...
Delay Server需要将消息写入磁盘，磁盘性能和机器空闲内存是影响其性能的关键因素，但一般delay的消息量不会太高，所以普通虚拟机的IO也可以，不需要物理机。

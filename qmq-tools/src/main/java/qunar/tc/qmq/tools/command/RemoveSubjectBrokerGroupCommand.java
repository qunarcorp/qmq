 ...
 * @since 2018-12-05
 */
@Command(name = "RemoveSubjectBrokerGroup", mixinStandardHelpOptions = true, sortOptions = false)
public class RemoveSubjectBrokerGroupCommand implements Runnable {
    ...
    @Option(names = {"--subject"}, required = true, description = {"one subject or *, * means all subjects"})
    private String subject;

    public RemoveSubjectBrokerGroupCommand(final MetaManagementService service) {
    ...
    public void run() {
        ...
        params.put("action", "RemoveSubjectBrokerGroup");
